...
- Install or update scripts (`install`, `dots`, mise tasks) that run untrusted
  input or fetch artifacts over insecure channels.
- Workflow or token-permission weaknesses in `.github/workflows/`.
- Secrets or credentials committed to the repository.

## Supported versions

Only the current state of `main` receives fixes. No releases or backports exist.
